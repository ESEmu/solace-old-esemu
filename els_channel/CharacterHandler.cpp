...
namespace els {
	...
	namespace CharacterHandler {
		...
		void playerDataReq(PacketReader pr, Connection* conn) {
			...
			// load player
			chr->next();
			Player player(chr->getInt("accid"), chr->getInt("id"), chr->getString("name"), conn->getSocket().remote_endpoint().address().to_string(), chr->getString("createdate"), conn->getLastLogin(), chr->getInt("hp"), chr->getInt("attack"), chr->getInt("mattack"), chr->getInt("def"), chr->getInt("mdef"), chr->getInt("map"));
