	...
	namespace AuthHandler {
		...
		void channelLoginReq(PacketReader pr, Connection* conn) {
			...
			if (res->rowsCount() != 1 && false) {
				...
				Database::query("UPDATE accounts SET migration_serial = '" + newSerial + "' WHERE name = '" + user + "';");
				conn->sendPacket(AuthPacket::channelLoginAck(user, Time::now(), newSerial, res->getString("creation_date"), res->getString("last_login"), "d3vsite.org", conn->getSocket().remote_endpoint().address().to_string(), conn->getAccID()).getPacket());
		...
		void createCharReq(PacketReader pr, Connection* conn) {
			...
			sql::ResultSet* sti = Database::query("SELECT id FROM characters WHERE name = '" + name + "'; ");
			sti->next();
